...
for scenario in scenario_list:
    ...
    for tick in [1]:
        ...
        for x_i, x in enumerate(xs):
            ...
            for y_i, y in enumerate(ys):
                ...
                if (
                    ...
                    v_q3 = set_in_range(ret_gnn[2].item(), 0., 1.,) * 255
                else:
                    if white_zone:
                        v_q1 = 255
                        v_q2 = 255
        ...
        z_q3 = z_q3.astype(np.uint8)
        z_q3 = cv2.flip(z_q3, 0)
        resized_q3 = cv2.resize(z_q3, (640, 640), interpolation=cv2.INTER_NEAREST)
        resized_q3 = beautify_image(cv2.cvtColor(resized_q3, cv2.COLOR_GRAY2BGR))
        ...
        final_img_q3 = cv2.warpAffine(resized_q3, Mr, (rows, cols),borderValue = (255, 255, 255))

        cv2.imwrite(dst_str_a + "clean" + dst_str_b_q1, final_img_q1)
        cv2.imwrite(dst_str_a + "clean" + dst_str_b_q2, final_img_q2)
        cv2.imwrite(dst_str_a + "clean" + dst_str_b_q3, final_img_q3)
        ...
        for human in data_sequence[-1]["people"]:
            ...
            resized_q3 = cv2.circle(resized_q3, center_coordinates, radius, color, thickness)            
        ...
        for human in data_sequence[-1]["people"]:
            ...
            resized_q3 = cv2.line(resized_q3, center_coordinates, (center_coordinates[0]+vx, center_coordinates[1]+vy), color, thickness)            
        ...
        cv2.imwrite(dst_str_a + "filled" + dst_str_b_q3, final_img_q3)        
